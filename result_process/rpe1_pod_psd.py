...
def extract_row(results_df, model, seed):
    model_dict = {
        'Ctrl': f'ctrl_mean_seed_{seed}_data_replogle_rpe1_essential',
        'CPA': f'CPA_seed_{seed}_data_replogle_rpe1_essential',
        'GEARS': f'GEARS_seed_{seed}_data_replogle_rpe1_essential',
...
def process_results(results_df):
    ...
    for seed in seeds:
        ...
        new_dfs[f'rpe1_split_{seed}_pod_psd'] = pd.DataFrame(new_df)


    for k, df in new_dfs.items():
        df.to_csv(f'csvs/{k}.csv')

csv_raw_file = 'csvs/rpe1_split_and_ctrl_manual.csv'
...
data = {}
for i in range(1, 6):
    file_name = f'csvs/rpe1_split_{i}_pod_psd.csv'
